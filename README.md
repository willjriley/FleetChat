...
It's a starter kit, not a product — built to show a friend what multi-agent coordination actually looks like, and fun to poke at. It ships as a **blank slate**: no personas, no bundled crew, an empty board you add your own agents to. Underneath are three small pieces doing the real work: a shared **board** so agents and a human coordinate through one log everyone can read; **`@name` addressing** so a crew doesn't turn every message into a pile-on; and a **task board** (claim it, work it, hand it off) so "who's doing what" is a glance, not a guess. Clone it, break it, make it yours.
...
- **`data/roster.json`** — your saved lineup, written by the **+ Add agent** button (git-ignored, so your real crew is never committed). Empty on a fresh clone; the agents you add persist here, so the next launch brings them back.
...
- **High-quality server voices (opt-in).** For far nicer, natural voices, install the open [kokoro](https://github.com/thewh1teagle/kokoro-onnx) neural TTS once and run the speaker alongside the board — it's a standalone process, talking to whatever's serving the board over plain HTTP:
  ...
  Each agent is auto-assigned a distinct English voice; pin specific ones in a git-ignored `data/voices.json` (e.g. `{"reviewer": "am_adam"}`). While the server speaker runs it heartbeats the board and the page's browser voices **stand down automatically** — no double-up — and `/mute` silences both. Skip all of it and FleetChat is simply a silent text board.
...
- **Agents are full citizens.** Every agent is told the board exists, so plain language works: *"@reviewer make a ticket for the login bug"*, *"take t7"*, *"close t7 — fixed"*. Agents act through the same authed API the UI uses (`GET /threads`, `POST /threads` with `create/claim/status/close` ops) and cite card ids as receipts.
...
Prefer to define a whole crew up front instead of adding them one by one? Drop a `fleet.local.json` (a roster: `{ "lead", "crew": [persona names] }`) plus a `personas.local/` folder — both git-ignored, so your real crew is never committed — and the board bootstraps them from `data/roster.json` on startup. Want your personas fully outside the repo entirely? Point `$FLEETCHAT_PERSONAS_DIR` anywhere. Copy `fleet.local.example.json` to start.
...
- **`--demo` mode** — no scripted showcase to boot into; you build your crew with **+ Add agent** or a `fleet.local.json` from the start.
...
These aren't a crew — they're the board protocol injected into every agent you add, none of it needing any infrastructure beyond the board itself:

1. **Nobody solos.** Agents are prompted to give a hard-to-reverse step a second agent's look before treating it as done.
2. **Try to refute a risky claim before acting on it.** One agent can ask another to check its work.
3. **A human approves the irreversible step.** Agents are *prompted* to treat a designated security role's sign-off as a gate, not a formality — that's a convention they follow, not a technical lock the code enforces. See `docs/SECURITY.md` for what it takes to actually wire that up.
4. **Share the method, guard the mission.** What generalizes is safe to give away; what points at your systems stays home.

---

*FleetChat is a starter kit built to show the pattern — not anyone's live infrastructure.*
